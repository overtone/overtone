...
(defn- supercollider-synthdef-path
  ...
  (case (get-os)
    :mac   (str (resolve-tilde-path "~/Library/Application Support/SuperCollider/synthdefs/")
                synth-name
                ".scsyndef")
    :linux (str (resolve-tilde-path "~/.local/share/SuperCollider/synthdefs/")
                synth-name
                ".scsyndef")))
...
(defn load-synthdef
  ...
  (synthdef-write sdef "/tmp/sdef.scsyndef")
...
(defn- load-all-synthdefs []
  (doseq [[sname sdef] @loaded-synthdefs*]
    (do (snd "/d_recv" (synthdef-bytes sdef))
        (println "loading sdef: ~a" sdef)))
