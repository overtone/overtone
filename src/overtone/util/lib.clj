...
(defn uuid
  "Creates a random, immutable UUID object that is comparable using
  the '=' function."
  []
...
(defn callable-map
  ...
  called as a fn from lookup to an arbitrary fn you specify at
  creation time."
...
(defmacro defrecord-ifn
  ...
  It generates all arities of invoke, calling the function.  Besides
  generating the clojure.lang.IFn implementation, you can declare any
  other implementations as you would normally with defrecord."
...
(defn arg-mapper
  "Takes a list of args, expected arg names and map of defaults.
   Creates a map of arg names to args using the defaults as the
   starting point.

   The args should be passed in the order 'ordered params', 'keyword
   params' such as the following: [1 2 3 :d 4 :f 5] where 1 2 and 3
   are ordered params and 4 and 5 are named params (associated with :d
   and :f respectively).

   If the expected args is the list [:a :b :c :d :f] then the
   resulting map will look as follows: {:a 1 :b 2 :c 3 :d 5 :f 5}. If
   the defaults contains extra keys, these will be merged in with any
   clashes being overridden with the result map, so if the default map
   is {:a 99 :h 2} the final output will be {:a 1 :b 2 :c 3 :d 4 :f
   5 :h 2}.
...
(defn invert-map
  "Takes a map m and returns a new map that's keys are the m's vals
  and that's vals are m's keys. Assumes that m's keys and vals are
  both sets (i.e. don't contain any duplicates). If there are
  duplicate values in the map they will result in key collisions and a
  smaller result map.
...
(defn update-every-n
  "Update every nth element in a seq of maps, optionally offset from
  the start, by applying f to the current value.

  (update-every-n [{:a 1} {:a 2} {:a 3} {:a 4} {:a 5}]
                  2 1 :a #(* 10 %))
...
(defn- welcome-message
  [user-name]
  (let [opts [
              (str "Hello " user-name ", may this be the start of a beautiful music hacking session...")
              (str "Cometh the hour, cometh the overtone hacker: " user-name ".")
              (str "Hello " user-name ", may algorithmic beauty pour forth from your fingertips.")
              (str "Hey " user-name ", sonic brilliance is only a few functions away...")
              (str "Hello " user-name ", just pause for a brief moment to summon your creative powers...")
              (str "Hello " user-name ", hold tight and feel that creativity pump through your veins...")
              (str "Wub wub sschhrrrweeeeeexxxx Hey " user-name "! kkrrrwwwwuggggg wub wub wub...")]]
...
(defn overtone-ugen-name
  ...
  SuperCollider names to Overtone names. Most likely needs
  improvement.
...
(defn resolve-gen-name
  "If the gen is a cgen or ugen returns the :name otherwise returns
   name unchanged assuming it's a keyword."
  [gen]
