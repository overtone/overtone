...
(defn synth-controller
  "Create a GUI for the given synths or instruments.  Each synth must
   have sufficient parameter metadata in the synthdef.

    (defsynth foo [freq {:default 440 :min 20 :max 10000 :step 1}]
      (out 0 (* [0.6 0.6] (env-gen (perc 0.01 0.2) :action FREE) (sin-osc freq))))

   Pops up gui window that will modify active synths and default
   param values
